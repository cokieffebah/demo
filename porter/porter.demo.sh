...
echo "Bringing up Notary and Docker Registry locally"
    export GOPATH=~/go
    mkdir -p ~/go/src/github.com/theupdateframework
...
echo "Signing Bundle and push trust data to local notary server"
    ...
    #--in-toto --layout testdata/intoto/root.layout --links testdata/intoto --layout-key testdata/intoto/alice.pub

    #docker push bundle to registry
...
echo "Verifying Bundle"
    ...
    #install bundle, using helm
    porter install --cred test-bundle